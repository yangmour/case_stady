...
export default {
  ...
  data() {
    return {
      ...
      validateRules: {
        ...
        postId: [{ required: true, trigger: 'blur', message: '岗位必须选择' }],
        deptId: [{ required: true, trigger: 'blur', message: '部门必须选择' }],
      },
  ...
  created() {
    ...
    postApi.findAll().then((response) => {
      this.postList = response.data
    })
    deptApi.findUserNodes().then((response) => {
  ...
  methods: {
    ...
    getRoles() {
      roleApi.getRolesByUserId(this.sysUser.id).then((response) => {
