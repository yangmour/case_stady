...
public class HospitalController {
	...
	public Result submitOrder(HttpServletRequest request, HttpServletResponse response) {
		try {
	...
	public Result updateCancelStatus(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			Map<String, Object> resultMap = hospitalService.updateCancelStatus(paramMap);
			return Result.ok(resultMap); //返回当前的余号数据
		} catch (YyghException e) {
			return Result.fail().message(e.getMessage());
		}
	}
