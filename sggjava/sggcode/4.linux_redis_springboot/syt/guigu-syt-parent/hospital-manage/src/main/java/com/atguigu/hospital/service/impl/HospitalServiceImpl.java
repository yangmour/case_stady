...
public class HospitalServiceImpl implements HospitalService {
    ...
    public Map<String, Object> submitOrder(Map<String, Object> paramMap) {
        ...
        if(availableNumber > 0) {
            ...
            //预约记录唯一标识（医院预约记录主键）
            resultMap.put("hosOrderId", orderInfo.getId());
            //预约号序
    ...
    public void updatePayStatus(Map<String, Object> paramMap) {
        String hosOrderId = (String)paramMap.get("hosOrderId");

        OrderInfo orderInfo = orderInfoMapper.selectById(hosOrderId);
        if(null == orderInfo) {
            throw new YyghException(ResultCodeEnum.DATA_ERROR);
        }
        //已支付
    ...
    @Override
    public Map<String, Object> updateCancelStatus(Map<String, Object> paramMap) {
        String hosOrderId = (String)paramMap.get("hosOrderId");
        ...
        //获取排班信息
        Schedule schedule = this.getSchedule(orderInfo.getScheduleId().toString());
        if(null == schedule) {
            throw new YyghException(ResultCodeEnum.DATA_ERROR);
        }
        ...
        int availableNumber = schedule.getAvailableNumber().intValue() + 1;
        schedule.setAvailableNumber(availableNumber);
        hospitalMapper.updateById(schedule); //更新医院端的排班数据
